...
def is_prime(n):
    if n < 2:
        return False
    for i in range(2, int(np.sqrt(n)) + 1): # Aby sprawdzić, czy liczba n jest liczbą pierwszą,
...
plt.scatter(*zip(*[(x,y) for x, y, _ in spiral]), color="lightgray", s=1, label="Liczby całkowite")
