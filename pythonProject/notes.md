...
### Wykres największej temperatury z pliku csv
```commandline
import csv
...
### Wykres najwyższych i najniższych z kolorowaniem obszaru
```commandline
import csv
from datetime import datetime
...
plt.show()
```
### Odczytywanie nagłówków z pliku wraz z ich indeksem
```commandline
import csv
...
### Obsłużenie wyjątku, w przypadku, gdy jest pusta wartość i nie można jej skonwertować na int
```commandline
    for row in reader:
        current_date = datetime.strptime(row[2], "%Y-%m-%d") # konwersja daty z tekstowej
        try:
